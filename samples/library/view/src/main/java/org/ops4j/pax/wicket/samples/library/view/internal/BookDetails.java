...
public final class BookDetails extends LibraryHomePage
{
    ...
    public BookDetails( long id )
    {
        ...
        List<Book> related = book.getRelatedBooks();
        for( Book relatedbook : related )
        {
            String title = getLocalizer().getString( "noBookTitle", this );
            BookmarkablePageLink child = link( "related", relatedbook, title );
            add( child );
        }
        ...
        if( hasStyles )
        {
            ...
            for( WritingStyle writingStyle : book.getWritingStyles() )
            {
     ...
     */
    public static BookmarkablePageLink link( final String name, final Book book, final String noBookTitle )
    {
